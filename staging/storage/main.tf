...
resource "aws_ebs_volume" "staging-storage-0" {
  ...
  tags {
    Description = "${var.environment} storage set 0"
    Name = "${var.environment}-storage-0-${count.index}"
    ManagedBy = "terraform"
    Repo = "${var.repository}"
    Organisation = "${var.organisation}"
    Environment = "${var.environment}"
...
resource "aws_ebs_volume" "staging-storage-1" {
  count = "${var.storage-cluster-zones}"
  availability_zone = "${element(data.aws_availability_zones.available.names, count.index)}"
  size = "${var.storage-gigabytes}"
  tags {
    Description = "${var.environment} storage set 1"
    Name = "${var.environment}-storage-1-${count.index}"
    ManagedBy = "terraform"
    Repo = "${var.repository}"
    Organisation = "${var.organisation}"
    Environment = "${var.environment}"
...
resource "aws_volume_attachment" "staging-storage-1-attachment" {
  count = "${var.storage-cluster-zones}"
  device_name = "${var.brick-1-device}"
  volume_id   = "${aws_ebs_volume.staging-storage-1.*.id[count.index]}"
  instance_id = "${aws_instance.staging-storage-node.*.id[count.index]}"
  # Fix for https://github.com/terraform-providers/terraform-provider-aws/issues/2084.
  provisioner "remote-exec" {
    when = "destroy"
